...
// 1. Generar ID y UI Inicial (Para que NUNCA se quede en negro si falla el 3D)
...
try {
    ...
    document.getElementById('status').innerText = '⚠️ Error en QR';
}

// Map of peerId -> { conn, name }
const connections = {};
function isAnyConnected() { return Object.keys(connections).length > 0; }
function updatePlayersUI() {
    ...
    if (count === 0) {
        ...
        list.innerHTML = Object.values(connections).map(c => `<span class="player-tag">📱 ${c.name}</span>`).join('');
    }
}
...
try {
    ...
    document.getElementById('status').innerText = '⚠️ Error 3D (¿WebGL soportado?) pero QR sigue activo.';
}
